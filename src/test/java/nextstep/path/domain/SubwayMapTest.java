...
class SubwayMapTest {
    ...
    void setUp() {
        ...
        이호선 = LineFactory.createLine(1L, "1호선", "green", 0L, 교대역_강남역_구간);
        신분당선 = LineFactory.createLine(2L, "신분당선", "red", 0L, 강남역_양재역_구간);
        삼호선 = LineFactory.createLine(3L, "2호선", "orange", 0L, 교대역_남부터미널_구간);
    ...
    void findShortestDistancePathTest() {
        final Optional<Path> shortestPathOptional = subwayMap.findShortestPath(강남역, 남부터미널역, PathType.DISTANCE);
        ...
        assertSoftly(softly -> {
            ...
            softly.assertThat(shortestPath.getUsedLine()).containsExactlyInAnyOrder(이호선, 삼호선);
        });
    }
    ...
    void findShortestPathNotFoundTest() {
        assertThatThrownBy(() -> subwayMap.findShortestPath(강남역, 서울역, PathType.DISTANCE))
    ...
    void findShortestDurationPathTest() {
        final Optional<Path> shortestPathOptional = subwayMap.findShortestPath(강남역, 남부터미널역, PathType.DURATION);
        ...
        assertSoftly(softly -> {
            ...
            softly.assertThat(shortestPath.getUsedLine()).containsExactlyInAnyOrder(신분당선, 삼호선);
        });
    }
}
