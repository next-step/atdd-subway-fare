...
class AuthAcceptanceTest extends AcceptanceTest {
    ...
    /**
     * When 만료된 JWT으로 인증이 필요한 API를 요청한다.
     * Then 401 코드를 리턴한다.
     */
    @DisplayName("만료된 JWT토큰으로 요청하면 401코드를 리턴한다.")
    @Test
    void bearerAuth_expired_jwt() {
        회원_생성_요청(사용자1.email(), PASSWORD, AGE);

        final ExtractableResponse<Response> response = 만료된_JWT로_개인정보_요청(사용자1.email());
