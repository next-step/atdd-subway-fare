...
public class PathFinderStepDef implements En {
    ...
    public PathFinderStepDef() {
        ...
        When("{string}에서 {string}까지 경로를 조회할 경우", (String departure, String arrival) -> {
            컨텍스트.저장된_응답 = 성공하는_지하철_경로_조회_요청(지하철_경로(
                    컨텍스트.저장소_숫자_가져오기(departure),
                    컨텍스트.저장소_숫자_가져오기(arrival)));
        });
        ...
        Then("최단거리 값은 {int}이다.", (Integer distance) -> {
