...
import static nextstep.member.acceptance.AuthStep.로그인;
import static nextstep.member.acceptance.MemberSteps.회원_생성_요청;
import static nextstep.subway.acceptance.LineSteps.지하철_노선_생성_요청;
import static nextstep.subway.acceptance.LineSteps.지하철_노선에_지하철_구간_생성_요청;
import static nextstep.subway.acceptance.PathStep.지하철_경로_조회;
import static nextstep.subway.acceptance.PathStep.지하철_경로_조회__로그인_유저;
...
public class PathStepDef implements En {
    ...
    public PathStepDef() {
        ...
        And("로그인 유저가 존재한다", (DataTable table) -> {
            List<Map<String, String>> maps = table.asMaps();
            maps.forEach(it -> {
                회원_생성_요청(
                    it.get("email"),
                    it.get("password"),
                    Integer.parseInt(it.get("age"))
                );

                context.store.put(it.get("age"),
                    로그인(it.get("email"),
                        it.get("password")).jsonPath().getString("accessToken")
                );
            });
        });
        ...
        When("{string} 과 {string} 경로를 {string}살이 조회하면,", (String from, String to, String age) -> {
            context.response = 지하철_경로_조회__로그인_유저(
                Long.parseLong(context.store.get(from).toString()),
                Long.parseLong(context.store.get(to).toString()),
                PathSearchType.DISTANCE,
                context.store.get(age).toString()
            );
        });
        ...
        Then("{string} 과 {string} 간 경로가 조회 된다.", (String from, String to) -> {
            final List<Station> stations = context.response.jsonPath()
    ...
    private Map<String, String> createSectionCreateParams(String upStationId, String downStationId,
