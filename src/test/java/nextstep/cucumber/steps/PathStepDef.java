...
public class PathStepDef implements En {
	...
	public PathStepDef() {
		Given("지하철 역들이 생성되어 있다:", (DataTable stationsTable) -> {
		...
		And("지하철 노선들이 생성되어 있다:", (DataTable linesTable) -> {
		...
		And("다음 구간이 추가되어 있다:", (DataTable sectionsTable) -> {
		...
		When("교대역에서 양재역까지의 최단 경로를 조회하면", () -> {
			response = executeFindPathRequest(1L, 3L);
		});

		Then("최단 경로가 정확하게 반환된다:", (DataTable expectedPathTable) -> {
