    ...
    public LineStepDef() {
        Given("지하철 노선들을 생성 요청하고", (final DataTable table) -> {
            ...
            maps.forEach(data -> {
                ...
                final ExtractableResponse<Response> response = LineApiHelper.createLine(lineName, lineColor, upStation.getId(), downStation.getId(), distance, 1);
        ...
        Given("지하철 구간을 등록 요청하고", (final DataTable table) -> {
            ...
            maps.forEach(data -> {
                ...
                SectionApiHelper.createSection(line.getId(), upStation.getId(), downStation.getId(), distance, 1);
            });
        });
    }
