...
public class PathAcceptanceTest extends AcceptanceTest {
  ...
  public void setUp() {
    ...
    이호선 = 지하철_노선_생성("2호선", "초록", 강남역.getId(), 역삼역.getId(), 3, 1);
    지하철_구간_생성_요청(이호선.getId(), 역삼역.getId(), 선릉역.getId(), 1, 3);

    신분당선 = 지하철_노선_생성("신분당선", "빨강", 강남역.getId(), 양재역.getId(), 1, 1);

    분당선 = 지하철_노선_생성("분당선", "노랑", 선릉역.getId(), 한티역.getId(), 1, 1);
    지하철_구간_생성_요청(분당선.getId(), 한티역.getId(), 도곡역.getId(), 1, 1);

    삼호선 = 지하철_노선_생성("3호선", "주황", 양재역.getId(), 매봉역.getId(), 1, 1);
    지하철_구간_생성_요청(삼호선.getId(), 매봉역.getId(), 도곡역.getId(), 1, 1);

    부산선 = 지하철_노선_생성("부산선", "검정", 남포역.getId(), 서면역.getId(), 5, 5);
  }
