...
class PathAcceptanceTest extends AcceptanceTest {
    ...
    /**
     * When : 두 역에 대해 거리를 기반으로 조회를 요청하면 Then : 거리가 가까운 순으로 조회가 되고 노선 추가 요금이 더해진다.
     */
    ...
    /**
     * When : 두 역에 대해 걸리는 시간을 기반으로 조회를 요청하면 Then : 시간이 적게 걸리는 순으로 조회가 되고 요금은 최단거리 기준으로 계산되고 노선 추가
     * 요금이 더해진다.
     */
    ...
    void findPathByDurationAndFare() {
        ...
        assertThat(response.jsonPath().getList("stations.id", Long.class)).containsExactly(교대역, 강남역,
                양재역);
        assertThat(response.jsonPath().getLong("fare")).isEqualTo(1850);
    }

    /**
     * Given : 청소년으로 로그인 했을때 When : 두 역에 대해 걸리는 시간을 기반으로 조회를 요청하면 Then : 시간이 적게 걸리는 순으로 조회가 되고 요금은
    ...
    void findPathByDurationAndYouthDiscountFare() {
        ...
        assertThat(response.jsonPath().getList("stations.id", Long.class)).containsExactly(교대역, 강남역,
                양재역);
        assertThat(response.jsonPath().getLong("fare")).isEqualTo(1200);
    }

    /**
     * Given : 어린이로 로그인 했을때 When : 두 역에 대해 걸리는 시간을 기반으로 조회를 요청하면 Then : 시간이 적게 걸리는 순으로 조회가 되고 요금은
    ...
    void findPathByDurationAndChildDiscountFare() {
        ...
        assertThat(response.jsonPath().getList("stations.id", Long.class)).containsExactly(교대역, 강남역,
                양재역);
        assertThat(response.jsonPath().getLong("fare")).isEqualTo(750);
    }
