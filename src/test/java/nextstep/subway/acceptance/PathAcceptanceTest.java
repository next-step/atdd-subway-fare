...
class PathAcceptanceTest extends AcceptanceTest {
    ...
    // Given 어린이 사용자가 로그인을 하고
    // When 경로를 조회하면
    // Then 어린이 할인이 적용된 금액을 확인할 수 있다.
    @DisplayName("로그인 사용자(어린이)가 두 역의 최단 거리 경로를 조회한다.")
    @Test
    void findPathWithLoginChild() {
        String accessToken = 깃허브_인증_로그인_요청(GithubResponses.어린이_사용자.getCode())
                .jsonPath()
                .getString("accessToken");

        var response = 로그인_사용자가_경로_조회를_요청(accessToken, 교대역, 양재역);

        assertThat(response.jsonPath().getInt("cost")).isEqualTo(800);
    }

    // Given 청소년 사용자가 로그인을 하고
    // When 경로를 조회하면
    // Then 청소년 할인이 적용된 금액을 확인할 수 있다.
    @DisplayName("로그인 사용자(청소년)가 두 역의 최단 거리 경로를 조회한다.")
    @Test
    void findPathWithLoginYouth() {
        String accessToken = 깃허브_인증_로그인_요청(GithubResponses.청소년_사용자.getCode())
                .jsonPath()
                .getString("accessToken");

        var response = 로그인_사용자가_경로_조회를_요청(accessToken, 교대역, 양재역);

        assertThat(response.jsonPath().getInt("cost")).isEqualTo(500);
    }
