...
public class PathAcceptanceTest {
	@BeforeAll
	void setup() {
		구간_생성_요청(수서역, 학여울역, 10, 5, 분당선);
		구간_생성_요청(수서역, 개포동역, 8, 5, 삼호선);
	}
	 ...
	 *       상행역 : 개포동역 / 하행역 : 수서역 / 길이 : 8  / 시간 : 5인 구간들이 존재하는 노선을 생성한다.
	 * When 수서역부터 도곡역까지의 경로를 조회하면
	 * Then 경로에 있는 역 목록은 수서역, 개포동, 도곡역 순서대로 구성된다.
	 * Then 구간의 거리는 12이다.
	 */
	@DisplayName("다건인 경로 중 최단 거리 경로를 조회한다.")
	@Test
	void 최단_거리_경로_조회() {
		// when & then
		최단_경로_조회_성공(최단_경로_조회_요청(수서역, 도곡역, PathType.DISTANCE), 12, 수서역, 개포동역, 도곡역);
	}
	 ...
	 * When 수서역부터 도곡역까지의 경로를 조회하면
	 * Then 경로에 있는 역 목록은 수서역, 개포동, 도곡역 순서대로 구성된다.
	 * Then 구간의 최소 거리는 12이다.
	 */
	@DisplayName("다건인 경로 중 최단 시간 경로를 조회한다.")
	@Test
	void 최단_시간_경로_조회() {
		// when & then
	 ...
	 * Then 구간의 최소 시간는 10이다.
	 */
	@DisplayName("노선이 다른 경로 조회.")
	@Test
	void 노선이_다른_경로_조회() {
		// when & then
		최단_경로_조회_성공(최단_경로_조회_요청(학여울역, 개포동역, PathType.DISTANCE), 10, 학여울역, 도곡역, 개포동역);
	}
