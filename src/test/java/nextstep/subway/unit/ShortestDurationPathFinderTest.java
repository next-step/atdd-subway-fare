...
public class ShortestDurationPathFinderTest {
    ...
    void setUp() {
        ...
        이호선 = Line.builder().name("2호선").color("GREEN").upStation(강남역).downStation(교대역).distance(3L).duration(3L).build();
        신분당선 = Line.builder().name("신분당선").color("RED").upStation(강남역).downStation(양재역).distance(10L).duration(5L).build();
        삼호선 = Line.builder().name("3호선").color("ORANGE").upStation(교대역).downStation(남부터미널역).distance(2L).duration(10L).build();
