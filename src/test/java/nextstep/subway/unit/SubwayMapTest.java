...
class SubwayMapTest {
    ...
    void setUp() {
        ...
        신분당선.addSection(강남역, 양재역, 3, 3000);
        이호선.addSection(교대역, 강남역, 3, 100000000);
        삼호선.addSection(교대역, 남부터미널역, 5, 5000);
        삼호선.addSection(남부터미널역, 양재역, 5, 5000);
    }
    ...
    void findPathOppositelyDuration() {
        // given
        List<Line> lines = Lists.newArrayList(신분당선, 이호선, 삼호선);
        SubwayMap subwayMap = new SubwayMap(lines, SectionPathType.DURATION);

        // when
        Path path = subwayMap.findPath(양재역, 교대역);

        // then
        //path.getStations().forEach(it -> System.out.println(it.getName()));
