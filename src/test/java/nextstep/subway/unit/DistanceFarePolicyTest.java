...
class DistanceFarePolicyTest {
    ...
    void calculateFiveIntervalExtraFare (final int weight, final int totalFare) {
        final Path path = new Path(최단_구간_목록, weight, 0);
        final Fare expected = DistanceFarePolicy.from(path);
        assertThat(expected).isEqualTo(Fare.from(totalFare));
    }
}
