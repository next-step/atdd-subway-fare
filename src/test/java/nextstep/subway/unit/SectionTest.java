...
public class SectionTest {
    ...
    @DisplayName("구간 A -> C 에 A -> B를 추가하면 A -> B -> C의 시간은 A -> C와 동일하다")
    @Test
    void insertSectionDuration() {
        // given
        Station 시청 = stationStore.get("시청");
        Station 을지로3가 = stationStore.get("을지로3가");
        Line line = new Line("2호선", Color.GREEN, 시청, 을지로3가, 6, 10);
        ...
        Station 을지로입구 = stationStore.get("을지로입구");
        // when
        line.addSection(new Section(시청, 을지로입구, 4, 2, line));

        // then
        Assertions.assertThat(line.getDuration()).isEqualTo(10);
    }

    @DisplayName("A - B 구간에 B - C를 추가하면 A->B->C의 소요시간은 (A->B), (B->C) 구간의 합이 된다")
    @Test
    void appendSectionDuration() {
        ...
        // when
        Station 을지로3가 = stationStore.get("을지로3가");
        line.addSection(new Section(을지로입구, 을지로3가, 4, 4, line));

        // then
        Assertions.assertThat(line.getDuration()).isEqualTo(14);
    }
}
