...
class DurationPathFinderTest {
    ...
    void setUp() {
        ...
        이호선 = new Line("2호선", Color.ORANGE, 교대역, 강남역, 1, 1000000);
        신분당선 = new Line("신분당선", Color.ORANGE, 강남역, 양재역, 1, 19999);
        삼호선 = new Line("3호선", Color.ORANGE, 교대역, 남부터미널역, 1, 1);
        삼호선.addSection(new Section(남부터미널역, 양재역, 1, 10, 삼호선));
    }


    @Test
    @DisplayName("최소시간 경로를 반환한다")
    public void shouldFindShortestPath() {

        PathFinder pathFinder = new DurationPathFinder(List.of(이호선, 신분당선, 삼호선));
    ...
    @Test
    @DisplayName("최소시간 경로의 소요시간를 반환한다")
    public void shouldFindShortestDistance() {

        PathFinder pathFinder = new DurationPathFinder(List.of(이호선, 신분당선, 삼호선));

        PathsDto pathsDto = pathFinder.findPath(교대역, 양재역);
        assertThat(pathsDto.getDuration()).isEqualTo(11);
    }

    @Test
    @DisplayName("출발역과 도착역이 같은 경우는 최소시간 경로를 탐색할 수 없다")
    ...
    @DisplayName("출발역과 도착역이 연결이 되어 있지 않은 경우 최소시간 경로를 탐색할 수 없다")
    @Test
    ...
    public void isConnectedShouldReturnFalse_IfStationsAreConnected() {
        ...
        PathFinder pathFinder = new DurationPathFinder(List.of(신분당선, 삼호선));
