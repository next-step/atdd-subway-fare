...
public class PathAcceptanceTest extends AcceptanceTest {
    ...
    void findPathByDistance() {
        // when
        ExtractableResponse<Response> response = 두_역의_최단_거리_경로_조회를_요청(양재역.getId(),교대역 .getId(), 토큰.getAccessToken());
    ...
    void findPathByDuration() {
        // when
        ExtractableResponse<Response> response = 두_역의_최소_소요_시간_경로_조회를_요청(교대역.getId(), 양재역.getId(), 토큰.getAccessToken());

        // then
        경로_응답됨(response, Lists.newArrayList(교대역.getId(), 강남역.getId(), 양재역.getId()), 20, 20);
    }

    @DisplayName("두 역의 지하철 요금을 조회한다.")
    @Test
