...
public class PathSteps extends AcceptanceTestSteps {
    ...
    public static ExtractableResponse<Response> Path_경로조회_요청_최단거리(Long sourceStationId, Long targetStationId){
    ...
    public static ExtractableResponse<Response> Path_경로조회_요청_최소시간(Long sourceStationId, Long targetStationId){
        return PATH_GIVEN_SPEC설정_filter설정()
                .accept(MediaType.APPLICATION_JSON_VALUE)
                .queryParam("source", sourceStationId)
                .queryParam("target", targetStationId)
                .when().get("/pathsByTime")
                .then().log().all().extract();
    }
}
