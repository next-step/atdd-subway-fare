...
#
  Scenario: 성공 경로 조회시 출발역과 도착역이 연결되어 있을 경우 최소 거리 경로와 기본 요금을 조회할 수 있다
    ...
    And 최소 거리와 기본 요금 테스트 결과 최소 시간 기준 총 거리 10, 소요 시간 10를 함께 응답한다
    And 최소 거리와 기본 요금 테스트 결과 지하철 이용 요금인 1250원을 함께 응답한다
...
#
  Scenario: 성공 경로 조회시 출발역과 도착역이 연결되어 있을 경우 최소 시간 경로를 조회할 수 있다
    ...
    And 최소 시간 기준 지하철 이용 요금인 1350원을 함께 응답한다
