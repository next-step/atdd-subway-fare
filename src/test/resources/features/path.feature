Feature: 지하철 경로 관련 기능
  Background:
    ...
    Scenario: 6살 어린이는 350원 공제한 금액의 50% 할인
      When "남부터미널역" 과 "서초역" 경로를 "6"살이 조회하면,
      Then 거리는 20이다.
      Then 시간은 4이다.
      Then 요금은 1350원이다.

    Scenario: 12살 어린이는 350원 공제한 금액의 50% 할인
      When "남부터미널역" 과 "서초역" 경로를 "12"살이 조회하면,
      Then 거리는 20이다.
      Then 시간은 4이다.
      Then 요금은 1350원이다.

    Scenario: 13살 청소년은 350원 공제한 금액의 20% 할인
      When "남부터미널역" 과 "서초역" 경로를 "13"살이 조회하면,
      Then 거리는 20이다.
      Then 시간은 4이다.
      Then 요금은 1950원이다.

    Scenario: 18살 청소년은 350원 공제한 금액의 20% 할인
      When "남부터미널역" 과 "서초역" 경로를 "18"살이 조회하면,
      Then 거리는 20이다.
      Then 시간은 4이다.
      Then 요금은 1950원이다.

    Scenario: 6살 미만 유아는 무료
      When "남부터미널역" 과 "서초역" 경로를 "5"살이 조회하면,
      Then 거리는 20이다.
      Then 시간은 4이다.
      Then 요금은 0원이다.
