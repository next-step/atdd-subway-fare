Feature: 로그인 하지 않은 회원의 지하철 경로 조회

  Background: 기본운임 비용은 1,250 원으로 균등 적용된다.
  ...
  # 최소 소요시간 경로 조회
  # 조회 가능
  Scenario: 회원 정보 없이 지하철 최소 소요시간 경로를 조회할 수 있다.
    Given 지하철 역과 노선을 생성하고, 구간을 추가한다.
    ...
    When  "양재역"에서 "신설동역"까지 "최소 소요시간" 기준으로 경로를 조회할 경우
    Then  경로 내 존재하는 역, 이동 거리, 소요 시간 정보를 확인할 수 있다.
      | 양재역, 강남역, 잠실역, 신설동역 | 60 | 33 |
    And   기본운임 비용, 추가운임 비용, 노선 추가 요금이 포함된 요금 정보를 확인할 수 있다.
      | 1250 | 1000 | 400 |
