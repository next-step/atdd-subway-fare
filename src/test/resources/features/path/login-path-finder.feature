Feature: 로그인한 회원의 지하철 경로 조회
  ...
  Background: 기본운임 비용만 연령대별로 차등 적용된다.
  ...
  Scenario: 지하철 최단거리 경로를 조회할 수 있다.
    ...
    Given "청소년" 인 회원을 생성한다.
    When  "강남역"에서 "남부터미널역"까지 "최단거리" 기준으로 "청소년" 인 회원이 경로를 조회할 경우
    Then  경로 내 존재하는 역, 이동 거리, 소요 시간 정보를 확인할 수 있다.
      | 강남역, 교대역, 남부터미널역 | 12 | 12 |
    And   기본운임 비용, 추가운임 비용, 노선 추가 요금이 포함된 요금 정보를 확인할 수 있다.
      | 720 | 100 | 800 |
    ...
    Given "성인" 인 회원을 생성한다.
    When  "강남역"에서 "남부터미널역"까지 "최단거리" 기준으로 "성인" 인 회원이 경로를 조회할 경우
    Then  경로 내 존재하는 역, 이동 거리, 소요 시간 정보를 확인할 수 있다.
      | 강남역, 교대역, 남부터미널역 | 12 | 12 |
    And   기본운임 비용, 추가운임 비용, 노선 추가 요금이 포함된 요금 정보를 확인할 수 있다.
      | 1250 | 100 | 800 |
