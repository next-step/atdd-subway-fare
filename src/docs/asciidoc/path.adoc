[[path]]
:snippets: {docdir}/build/generated-snippets

== 지하철 경로

=== 최단 경로 조회

==== Request

include::{snippets}/path/http-request.adoc[]

==== Request Parameters

include::{snippets}/path/request-parameters.adoc[]

==== Response

include::{snippets}/path/http-response.adoc[]

==== Response Fields

include::{snippets}/path/response-fields.adoc[]


=== 최소 시간 경로 조회

==== Request

include::{snippets}/pathDuration/http-request.adoc[]

==== Request Parameters

include::{snippets}/pathDuration/request-parameters.adoc[]

==== Response

include::{snippets}/pathDuration/http-response.adoc[]

==== Response Fields

include::{snippets}/pathDuration/response-fields.adoc[]