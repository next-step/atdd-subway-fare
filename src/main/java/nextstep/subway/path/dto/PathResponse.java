...
public class PathResponse {
    ...
    public static PathResponse of(Path path, DistanceFarePolicies distanceFarePolicies, LineFarePolicy lineFarePolicy, DiscountPolicy discountPolicy) {
        List<StationResponse> stationResponses = path.getStations().stream()
                .map(StationResponse::from)
                .collect(Collectors.toList());

        return new PathResponse(stationResponses, path.getTotalDistance(), path.getTotalDuration(), path.calculateFare(distanceFarePolicies, lineFarePolicy, discountPolicy));
    }
