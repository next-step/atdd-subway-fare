...
public class MapController {
    ...
    public ResponseEntity<PathResponse> findPath(@AuthenticationPrincipal UserDetails userDetails, @RequestParam Long source, @RequestParam Long target,
        ...
        if (userDetails instanceof EmptyMember) {
            return ResponseEntity.ok(mapService.findPath(null, source, target, type));
        }

        return ResponseEntity.ok(mapService.findPath((LoginMember) userDetails, source, target, type));
    }
